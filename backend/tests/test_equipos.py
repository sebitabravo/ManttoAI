...
def test_get_provisioning_token_generates_valid_jwt(client):
    ...
    response = client.get("/api/v1/equipos/provisioning-token")
...
def test_create_equipo_persists_and_can_be_retrieved(client):
    ...
    assert create_response.headers["Location"].endswith(f"/api/v1/equipos/{created['id']}")
...
def test_update_equipo_persists_changes(client):
    ...
    create_response = client.post("/api/v1/equipos", json=_build_equipo_payload("Motor X"))
...
def test_delete_equipo_removes_record(client):
    ...
    create_response = client.post("/api/v1/equipos", json=_build_equipo_payload("Bomba Norte"))
    equipo_id = create_response.json()["id"]
