...
def test_latest_lectura_returns_last_persisted_record(client):
    """Valida que GET /api/v1/lecturas/latest/{equipo_id} lea última lectura real."""

    equipo_id = _create_equipo(client)
    client.post(
        "/api/v1/lecturas",
        json=_build_lectura_payload(equipo_id=equipo_id, temperatura=38.5),
    )
    client.post(
        "/api/v1/lecturas",
        json=_build_lectura_payload(equipo_id=equipo_id, temperatura=47.1),
    )
...
def test_get_lecturas_limit_legacy_retrocompatible(client):
    ...
    response = client.get(
        "/api/v1/lecturas", params={"equipo_id": equipo_id, "limit": 100}
    )
